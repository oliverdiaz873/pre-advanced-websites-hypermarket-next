...
describe('createAddressAction', () => {
  ...
  it('con token → reenvía al backend y NO serializa el JWT', async () => {
    mockedCookies.mockReturnValue(mockCookie(TOKEN) as never)
    ;(backend.createAddressRequest as jest.MockedFunction<typeof backend.createAddressRequest>).mockResolvedValue({ ok: true, status: 201, address })
...
describe('updateAddressAction', () => {
  ...
  it('con token e id → PATCH parcial al backend', async () => {
    mockedCookies.mockReturnValue(mockCookie(TOKEN) as never)
    ;(backend.updateAddressRequest as jest.MockedFunction<typeof backend.updateAddressRequest>).mockResolvedValue({ ok: true, status: 200, address })
...
describe('deleteAddressAction', () => {
  it('con token e id → DELETE al backend', async () => {
    mockedCookies.mockReturnValue(mockCookie(TOKEN) as never)
    ;(backend.deleteAddressRequest as jest.MockedFunction<typeof backend.deleteAddressRequest>).mockResolvedValue({ ok: true, status: 204 })
